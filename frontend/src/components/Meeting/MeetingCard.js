...
const MeetingCard = () => {
  ...
  return (
    <div>
        ...
        <div className="grid grid-cols-2 gap-20">
          {meetings.map((meeting) => (
            <Link to={`/meet/${meeting.meetNo}`}>
              <div className="rounded-lg w-[500px] shadow-xl bg-white h-40 flex items-center p-3" key={meeting.meetNo}>
      ...
      {pageInfo && Object.keys(pageInfo).length > 0 && <Pagination pageInfo={paginationInfo} onPageChange={handlePageChange} />}
    </div>
  );
};
